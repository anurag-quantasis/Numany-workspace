...
@Injectable({ providedIn: 'root' }) export class TenantPageService {
  ...
  getTenants(event: TableLazyLoadEvent) {
    ...
    return this.apiService.get(this.tenantsEndpoint, { params }).pipe(map((apiResponse: any) => {
      const paginatedData = {
  ...
  updateTenant(tenantToUpdate: Tenant): Observable<ApiResponse<Tenant>> {
    ...
    return this.apiService.put<ApiMutationResponse>(`this.tenantsEndpoint/${tenantToUpdate.id}`,apiPayload).pipe(
      map((apiResponse) => {
        if(!apiResponse.data){
          return {
            status: 'error',
