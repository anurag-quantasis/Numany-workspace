...
export const InterventionDataClassesStore = signalStore(
  ...
  withComputed((store) => ({})),
  // FIX: Restructure withMethods to maintain 'this' context
  withMethods((store, interventionDataClassesService = inject(InterventionDataClassesService)) => {
    // Define all methods in an object to create a closure
    const methods = {
        ...
        pipe(
          ...
          switchMap((classes) => {
            ...
            return interventionDataClassesService.addInterventionDataClasses(apiPayload).pipe(
              tap({
                next: (response) => {
                  // CORRECTED: Call the method via the 'methods' closure
                  methods.loadInterventionDataClasses();
                  patchState(store, {
                    isLoading: false,
                  });
                },
            ...
            interventionDataClassesService.getInterventionDataClassesById(id).pipe(
              tap({
                next: (classesArray) => {
                  ...
                  patchState(store, {
                    selectedInterventionDataCls: foundClasses,
                    isLoading: false,
                  });
                },
                error: (e) => patchState(store, { error: e.message, isLoading: false }),
