...
import { TenantLabAddResponse, TenantLabResult, TenantLabResultPayload, TenantLabResultResponse, TenantLabResultUpdateResponse } from "./lab-store.models";
...
export const TenantLabStore = signalStore(
  ...
  withMethods((store, tenantLabResultService = inject(TenantLabResultService), messageService = inject(MessageService)) => {
        ...
        switchMap(() =>
          tenantLabResultService.getTenantLabResults().pipe(
            tap((response: TenantLabResultResponse) => {
              ...
              if (response && response.data) { // Assuming a simple success check if `status` isn't present
    ...
    const updateTenantLabResult = rxMethod<TenantLabResult>(
      pipe(
        tap(() => patchState(store, { isLoading: true, error: null })),
        switchMap((labResult) => { // Start of switchMap's function body
